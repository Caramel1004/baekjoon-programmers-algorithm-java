...
class Solution {
    ...
    // 시간 초과
    private static int firstSolution(int num) {
        ...
        return Arrays.stream(nextBitPowers).sum();
    }
    
    // 통과
    // return Arrays.stream(nextBitPowers).sum() => 향상된 for문으로 누적 합 코드로 변경
    // 실행 시간 조건 있으면, 되도록 stream 변환 코드는 지양
    private static int secondSolution(int num) {
        int bitOnesCount = Integer.bitCount(num);
        // 1. 2의 거듭 제곱 수는 다음 거듭 제곱 수로 처리
        if (bitOnesCount == 1) {
            return num << 1;
