...
//입력 된 문자열의배열을 문자열의 길이순으로 정렬 ->중복 값 제거 하고 arrayList로 저장
//-> 중복 값 제거 -> 길이가 같은 것 끼리 그룹 배열을 만들어 사전순으로 정렬 후 list 수정 -> 출력 
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            if(list.size() <= index){//인덱스가 배열의 크기 보다 같거나 크면 tmpList정렬하고 루프 브레이크(조건이 끝나는 구간)
                ...
                tmpList.clear();
                break;
            }else if (list.get(index - 1).length() == list.get(index).length() && list.size() > index) {//문자열길이 같은거 tmpList에 저장 그리고 인덱스가 배열사이즈보다 작아야 함
